...
export default function BlogCarousel({ blogPosts }: Props) {
  const [curr, setCurr] = useState(0);

  const setPreviousSlide = () =>
    setCurr((curr) => (curr === 0 ? blogPosts.length - 1 : curr - 1));

  const setNextSlide = () =>
    setCurr((curr) => (curr === blogPosts.length - 1 ? 0 : curr + 1));
        ...
        {blogPosts.map((blogPost, i) => (
          <div className="w-full min-w-full flex flex-col items-center" key={i}>
            <div className="w-10/12 h-full items-center justify-center grid grid-cols-1 grid-flow-row-dense">
              <BlogCard blogPost={blogPost} key={i} />
            </div>
          </div>
        ))}
      </div>
